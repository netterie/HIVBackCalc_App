...
([SAS Extraction code](https://github.com/netterie/HIVBackCalc_App/tree/master/development))

right-click "Common Read In CBA.sas" to download the script to your computer.

  -Note that the SAS script requires that the eHARS data are in the "person" data file structurte. Future versions of HIVBackCalc will be able to work directly with the "pretest_questionnair" data files and automatically construct the person level data files.   
...
####    3.5 Uploading data to the HIVBackCalc App.
  ...
  2.  Click on the _choose file_ button on the left panel and select the data "data_KC_sim".
...
Click on the _Backcalculate infections_ tab at the top of the app to get to the Back calculation section.  Clicking on the _run backcalculation_ button on the left will run the back-calculation.  This may take a few moments. 

The plot and the table that are generated after the backcalculation is completed provide the same information in different formats.  Each includes the number of diagnosed cases during each quarter, the predicted incidence and the estimated number of undiagnosed cases.  The incidence and the number of undiagnosed cases are generate under the three different set of assumptions for calculating the TID.  As you can see in the plot below, in this example the different assumptions had little impact on the estimated incidence but a substantial impact on the estimates of the number of undiagnosed cases.     
